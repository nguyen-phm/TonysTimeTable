...
serve(async (req) => {
    ...
    if (action === "generateSecret") {
      try {
        const secret = speakeasy.generateSecret({ name: "Tony's Timetable", length: 20 });
        ...
        return new Response(JSON.stringify({ otpauth_url: secret.otpauth_url, base32: secret.base32 }), {
          headers: { "Content-Type": "application/json" },
        });
      } catch (err) {
